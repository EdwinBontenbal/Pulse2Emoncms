...
FILE="/tmp/Pulse2Emoncms_Watchdog"
LOGFILE="/var/log/Pulse2Emoncms_Watchdog.log"
PROCESS="python /usr/local/sbin/Pulse2Emoncms.py"
TimeNow=$(date +%s)
   ...
   then
	...
	if [ -z "$ProcessID" ];
	 ...
	 # No process found so start it
         WriteLog "No ProcessID belonging to Process($PROCESS) found, start it!"
